...
{//cloak this entity
	if ( self && self->client )
	{
		...
		{//cloak
			...
			//FIXME: debounce attacks?
			//FIXME: temp sound
			G_SoundOnEnt( self, CHAN_ITEM, "sound/movers/doors/forcefield_on.wav" );
		}
	}
}
...
{//decloak this entity
	if ( self && self->client )
	{
		...
		{//Uncloak
			...
			//self->client->ps.powerups[PW_UNCLOAKING] = level.time + 2000;
			//FIXME: temp sound
			G_SoundOnEnt( self, CHAN_ITEM, "sound/movers/doors/forcefield_off.wav" );
		}
	}
}

void Overload_On( gentity_t *self )
{//cloak this entity
	if ( self && self->client )
	{
		if ( !self->client->ps.powerups[PW_OVERLOADED] )
		{//cloak
			self->client->ps.powerups[PW_OVERLOADED] = Q3_INFINITE;
			//that's this?
			//self->client->ps.powerups[PW_UNCLOAKING] = level.time + 2000;
			//FIXME: debounce attacks?
			//FIXME: temp sound
			G_SoundOnEnt( self, CHAN_ITEM, "sound/weapons/overchargefast.wav" );
		}
	}
}

void Overload_Off( gentity_t *self )
{//decloak this entity
	if ( self && self->client )
	{
		if ( self->client->ps.powerups[PW_OVERLOADED] )
		{//Uncloak
			self->client->ps.powerups[PW_OVERLOADED] = 0;
			//self->client->ps.powerups[PW_UNCLOAKING] = level.time + 2000;
			//FIXME: temp sound
			G_SoundOnEnt( self, CHAN_ITEM, "sound/weapons/overchargeend.wav" );
		}
	}
}
...
static void Jedi_AggressionErosion( int amt )
{
	...
	if ( NPCInfo->stats.aggression < 5 || (NPCInfo->stats.aggression < 6&&NPC->client->NPC_class == CLASS_DESANN))
	{//turn off the saber
	...
	*/
	...
	{//we're out of striking range and we are allowed to attack
		...
		{//They're being gripped, rush them!
			...
			WP_ForcePowerAvailable( NPC, FP_GRIP, 0 ) && //I'm not busy
				...
				{//else, randomly try some kind of attack every now and then
					...
					//if ( (NPCInfo->rank == RANK_ENSIGN || NPCInfo->rank > RANK_LT_JG) && !Q_irand( 0, 1 ) )
					{
						...
						//else if ( WP_ForcePowerAvailable( NPC, FP_LIGHTNING, 0 ) && Q_irand( 0, 1 ) )
						{
							...
							&& Q_irand( FORCE_LEVEL_0, NPC->client->ps.fd.forcePowerLevel[FP_TEAM_FORCE] ) > FORCE_LEVEL_1
							&& WP_ForcePowerAvailable( NPC, FP_TEAM_FORCE, 0 ) 
							&& Q_irand( 0, 1 ) )
						{
...
static void Jedi_EvasionSaber( vec3_t enemy_movedir, float enemy_dist, vec3_t enemy_dir )
{
	...
	if ( (NPC->enemy->client->ps.fd.forcePowersActive&(1<<FP_TEAM_FORCE) ) )
	{//enemy is shooting lightning
		enemy_attacking = qtrue;
		shooting_lightning = qtrue;
		evasionChance = 50;
...
//RAFIXME - useDebounceTime not implimented
	...
	{//we won a saber lock, press the advantage with an attack!
		...
		{//based on skill with some randomness
			...
			NPC->client->ps.weaponTime = NPCInfo->shotTime = NPC->attackDebounceTime = level.time + 200;
			//NPC->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] = level.time + 500;
			NPC->client->ps.saberBlocked = BLOCKED_NONE;
			WeaponThink( qtrue );
			return qtrue;
			...
			{//have our heal target
				...
				*/
				...
				{//if we're healing Rosh, quit using force powers and face him.
					...
					WP_ForcePowerStop( NPC, FP_TEAM_FORCE );
					NPC_FaceEntity( NPC->client->leader, qtrue );
					return qtrue;
				}
