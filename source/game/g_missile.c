...
gentity_t *fire_grapple (gentity_t *self, vec3_t start, vec3_t dir)
{
	...
	if(self->client->skillLevel[SK_GRAPPLE] == FORCE_LEVEL_3)
	{
	VectorScale( dir, 3*m_grapple_shoot_speed.integer, hook->s.pos.trDelta ); // lmo scale speed!
	}
	else if(self->client->skillLevel[SK_GRAPPLE] == FORCE_LEVEL_2)
	{
	VectorScale( dir, 2*m_grapple_shoot_speed.integer, hook->s.pos.trDelta ); // lmo scale speed!
	}
	else
	{
	VectorScale( dir, m_grapple_shoot_speed.integer, hook->s.pos.trDelta ); // lmo scale speed!
	}		
