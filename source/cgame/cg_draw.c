...
void CG_DrawDodge( menuDef_t *menuHUD )
{
	...
	vec4_t			aColor;
	itemDef_t		*focusItem;
	float			percent = ((float)cg.snap->ps.stats[STAT_DODGE]/250.0)*DPBAR_H;
...
void CG_DrawForcePower( menuDef_t *menuHUD )
{
	...
	vec4_t			aColor;
	itemDef_t		*focusItem;
	float			percent = ((float)cg.snap->ps.fd.forcePower / 250.0f) * FPBAR_H;	
...
void CG_DrawJetpackFuel(void)
{
	...
	float x = JPFUELBAR_X;
	float y = JPFUELBAR_Y;
	float percent = ((float)cg.snap->ps.jetpackFuel/250.0f)*JPFUELBAR_H;
...
void CG_DrawCloakFuel(void)
{
	...
	float x = CLFUELBAR_X;
	float y = CLFUELBAR_Y;
	float percent = ((float)cg.snap->ps.cloakFuel/250.0f)*CLFUELBAR_H;
