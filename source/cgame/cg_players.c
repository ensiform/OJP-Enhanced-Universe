...
//		}
		...
		{ //broken right arm
				...
				(!ci->saber[1].model[0] || cent->currentState.weapon != WP_SABER))
			{
				...
				if (cent->currentState.weapon == WP_MELEE ||
					cent->currentState.weapon == WP_SABER ||
					cent->currentState.weapon == WP_BRYAR_PISTOL || cent->currentState.weapon == WP_BRYAR_OLD)
				...
				if (newAnimation != BOTH_MELEE1 &&
					...
					(newAnimation == TORSO_WEAPONREADY2 || newAnimation == BOTH_ATTACK2 || cent->currentState.weapon < WP_BRYAR_PISTOL	|| cent->currentState.weapon < WP_BRYAR_OLD))
				{
...
static void CG_ForceGraspEffect( vec3_t org )
{
	localEntity_t	*ex;
	...
	*/
	...
	ex->color[1] = 255;
	ex->color[2] = 255;
	ex->refEntity.customShader = cgs.media.greenSaberGlowShader;//trap_R_RegisterShader( "gfx/effects/forcePush" );
}
...
CheckTrail:   
    ...
    {// Use the supremely hacky SFX Sabers.
		...
		// Ensi: base, tip, dualbase, dualtip are all wrong checks here, they are arrays and these statements are always true
...
static void CG_ForceCryo( centity_t *cent, const vec3_t origin, vec3_t tempAngles, qhandle_t shader, qboolean alwaysDo )
{
	// Undoing for now, at least this code should compile if I ( or anyone else ) decides to work on this effect
	qboolean	found = qfalse;
	vec3_t		fxOrg, fxOrg2, dir;
	...
	if ( tr.fraction < 1.0f || random() > 0.94f || alwaysDo )
	{
	trap_FX_PlayEntityEffectID(cgs.effects.frozen, fxOrg, axis, cent->boltInfo, cent->currentState.number,-1,-1);
...
static void CG_ForceIncineration( centity_t *cent, const vec3_t origin, vec3_t tempAngles, qhandle_t shader, qboolean alwaysDo )
{
	// Undoing for now, at least this code should compile if I ( or anyone else ) decides to work on this effect
	qboolean	found = qfalse;
	vec3_t		fxOrg, fxOrg2, dir;
	...
	if ( tr.fraction < 1.0f || random() > 0.94f || alwaysDo )
	{
	trap_FX_PlayEntityEffectID(cgs.effects.burned, fxOrg, axis, cent->boltInfo, cent->currentState.number,-1,-1);
...
void *cg_g2Jetpack2Instance = NULL;

#define JETPACK2_MODEL "models/weapons2/jetpack_blue/model.glm"


void CG_InitJetpack2Ghoul2(void)
{
	...
	trap_G2API_InitGhoul2Model(&cg_g2Jetpack2Instance, JETPACK2_MODEL, 0, 0, 0, 0, 0);

	assert(cg_g2Jetpack2Instance);

	//Indicate which bolt on the player we will be attached to
	//In this case bolt 0 is rhand, 1 is lhand, and 2 is the bolt
	//for the jetpack (*chestg)
	trap_G2API_SetBoltInfo(cg_g2Jetpack2Instance, 0, 2);

	//Add the bolts jet effects will be played from
	trap_G2API_AddBolt(cg_g2Jetpack2Instance, 0, "torso_ljet");
	trap_G2API_AddBolt(cg_g2Jetpack2Instance, 0, "torso_rjet");
}

void CG_CleanJetpack2Ghoul2(void)
{
	if (cg_g2Jetpack2Instance)
	{
		trap_G2API_CleanGhoul2Models(&cg_g2Jetpack2Instance);
		cg_g2Jetpack2Instance = NULL;
	}
}
...
void CG_InitJetpack3Ghoul2(void)
{
	...
	trap_G2API_InitGhoul2Model(&cg_g2Jetpack3Instance, JETPACK3_MODEL, 0, 0, 0, 0, 0);

	assert(cg_g2Jetpack3Instance);

	//Indicate which bolt on the player we will be attached to
	//In this case bolt 0 is rhand, 1 is lhand, and 2 is the bolt
	//for the jetpack (*chestg)
	trap_G2API_SetBoltInfo(cg_g2Jetpack3Instance, 0, 2);

	//Add the bolts jet effects will be played from
	trap_G2API_AddBolt(cg_g2Jetpack3Instance, 0, "torso_ljet");
	trap_G2API_AddBolt(cg_g2Jetpack3Instance, 0, "torso_rjet");
}

void CG_CleanJetpack3Ghoul2(void)
{
	if (cg_g2Jetpack3Instance)
	{
		trap_G2API_CleanGhoul2Models(&cg_g2Jetpack3Instance);
		cg_g2Jetpack3Instance = NULL;
	}
}



void *cg_g2Jetpack4Instance = NULL;


#define JETPACK4_MODEL "models/weapons2/jetpack_metal/model.glm"

void CG_InitJetpack4Ghoul2(void)
{
	...
	trap_G2API_InitGhoul2Model(&cg_g2Jetpack4Instance, JETPACK4_MODEL, 0, 0, 0, 0, 0);

	assert(cg_g2Jetpack4Instance);

	//Indicate which bolt on the player we will be attached to
	//In this case bolt 0 is rhand, 1 is lhand, and 2 is the bolt
	//for the jetpack (*chestg)
	trap_G2API_SetBoltInfo(cg_g2Jetpack4Instance, 0, 2);

	//Add the bolts jet effects will be played from
	trap_G2API_AddBolt(cg_g2Jetpack4Instance, 0, "torso_ljet");
	trap_G2API_AddBolt(cg_g2Jetpack4Instance, 0, "torso_rjet");
}

void CG_CleanJetpack4Ghoul2(void)
{
	if (cg_g2Jetpack4Instance)
	{
		trap_G2API_CleanGhoul2Models(&cg_g2Jetpack4Instance);
		cg_g2Jetpack4Instance = NULL;
	}
}
...
void *cg_g2Jetpack5Instance = NULL;


#define JETPACK5_MODEL "models/weapons2/jetpack_white/model.glm"


void CG_InitJetpack5Ghoul2(void)
{
	...
	trap_G2API_InitGhoul2Model(&cg_g2Jetpack5Instance, JETPACK5_MODEL, 0, 0, 0, 0, 0);

	assert(cg_g2Jetpack5Instance);

	//Indicate which bolt on the player we will be attached to
	//In this case bolt 0 is rhand, 1 is lhand, and 2 is the bolt
	//for the jetpack (*chestg)
	trap_G2API_SetBoltInfo(cg_g2Jetpack5Instance, 0, 2);

	//Add the bolts jet effects will be played from
	trap_G2API_AddBolt(cg_g2Jetpack5Instance, 0, "torso_ljet");
	trap_G2API_AddBolt(cg_g2Jetpack5Instance, 0, "torso_rjet");
}

void CG_CleanJetpack5Ghoul2(void)
{
	if (cg_g2Jetpack5Instance)
	{
		trap_G2API_CleanGhoul2Models(&cg_g2Jetpack5Instance);
		cg_g2Jetpack5Instance = NULL;
	}
}
...
void CG_InitJetpack6Ghoul2(void)
{
	...
	trap_G2API_InitGhoul2Model(&cg_g2Jetpack6Instance, JETPACK6_MODEL, 0, 0, 0, 0, 0);

	assert(cg_g2Jetpack6Instance);

	//Indicate which bolt on the player we will be attached to
	//In this case bolt 0 is rhand, 1 is lhand, and 2 is the bolt
	//for the jetpack (*chestg)
	trap_G2API_SetBoltInfo(cg_g2Jetpack6Instance, 0, 2);

	//Add the bolts jet effects will be played from
	trap_G2API_AddBolt(cg_g2Jetpack6Instance, 0, "torso_ljet");
	trap_G2API_AddBolt(cg_g2Jetpack6Instance, 0, "torso_rjet");
}
...
void CG_CleanJetpack6Ghoul2(void)
{
	if (cg_g2Jetpack6Instance)
	{
		trap_G2API_CleanGhoul2Models(&cg_g2Jetpack6Instance);
		...
		{//have saber in inventory
			...
			{//saber is holstered. render as nessicary.
				...
				{//using single saber(s)
					...
					//secondary saber
					if(ci->saber[1].model[0] && ci->ghoul2Weapons[1])
					{//have a second saber
...
SkipTrueView:
	...
	if (cent->currentState.activeForcePass && cent->currentState.forcePowersActive & (1 << FP_DRAIN)
		&& cent->currentState.NPC_class != CLASS_VEHICLE)
	{
	...
	if (cent->currentState.activeForcePass && cent->currentState.forcePowersActive & (1 << FP_LIGHTNING)
		&& cent->currentState.NPC_class != CLASS_VEHICLE)
	{
	...
	//[/icethrower]
	else if( cent->currentState.userInt3 & (1 << FLAG_THROWER2))
	{//player is firing dioxisthrower, render effect.
		vec3_t axis[3];
		vec3_t tAng, fAng, fxDir;
		vec3_t efOrgL; //origin left hand
		vec3_t efOrgR; //origin right hand
	...
	else if(cent->currentState.userInt3 & (1 << FLAG_THROWER))
	...
	if( cent->currentState.userInt3 & (1 << FLAG_ADVANCEDTHROWER) && cent->currentState.userInt3 & (1 << FLAG_ADVANCEDTHROWER2))
	{//player is firing electroshocker, render effect.
		vec3_t axis[3];
		vec3_t tAng, fAng, fxDir;
		vec3_t efOrgL; //origin left hand
		vec3_t efOrgR; //origin right hand
	...
	else if( cent->currentState.userInt3 & (1 << FLAG_ADVANCEDTHROWER2))
	{//player is firing electroshocker, render effect.
		vec3_t axis[3];
		vec3_t tAng, fAng, fxDir;
		vec3_t efOrgL; //origin left hand
	...
	else if( cent->currentState.userInt3 & (1 << FLAG_ADVANCEDTHROWER))
	{//player is firing electroshocker, render effect.
		vec3_t axis[3];
		vec3_t tAng, fAng, fxDir;
		vec3_t efOrgL; //origin left hand
		...
		else
{
	...
	{
		trap_S_AddLoopingSound( cent->currentState.number, cg.refdef.vieworg, vec3_origin, 
			trap_S_RegisterSound( "sound/movers/doors/forcefield_lp.wav" ) 
			);
	}
	...
	{
		trap_S_AddLoopingSound( cent->currentState.number, cg.refdef.vieworg, vec3_origin, 
			trap_S_RegisterSound( "sound/weapons/overchargeloop.wav" ) 
			);
	}
	...
	if (cent->itemPowerEffectTime > cg.time)
	{
		...
		if ( random() > 0.4f )
		{
			...
			if ( random() > 0.9f )
				trap_S_StartSound ( NULL, cent->currentState.number, CHAN_AUTO, cgs.media.cryoSound );
		}
...
stillDoSaber:
	...
	*/
	...
	{//in the middle of cloaking
			...
			{
				CG_DrawPlayerSphere(cent, cent->lerpOrigin, 1.25f, cgs.media.ysaliredShader );
			}
			else 
			{
				CG_DrawPlayerSphere(cent, cent->lerpOrigin, 1.25f, cgs.media.ysaliblueShader );
	...
	//[/TrueView]
	{
	if (cent->currentState.userInt3 & (1 << FLAG_RAGE2))
	{
