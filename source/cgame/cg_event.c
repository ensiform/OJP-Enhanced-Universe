...
//	case HI_MEDKIT:
	...
	if (cg.snap && cg.snap->ps.clientNum == cent->currentState.number && itemNum != HI_BINOCULARS &&
		//[Flamethrower]
		itemNum != HI_JETPACK && itemNum != HI_SQUADTEAM  && itemNum != HI_VEHICLEMOUNT && itemNum != HI_MEDPAC && itemNum != HI_SHIELDBOOSTER && itemNum != HI_SHIELD && itemNum != HI_CLOAK && itemNum != HI_SEEKER && itemNum != HI_SENTRY_GUN && itemNum != HI_EWEB 
		&& itemNum != HI_FLAMETHROWER  && itemNum != HI_ELECTROSHOCKER && itemNum != HI_SPHERESHIELD && itemNum != HI_OVERLOAD && itemNum != HI_GRAPPLE)
...
//		trap_S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.noAmmoSound );
	...
	case EV_PREDEFSOUND:
		DEBUGNAME("EV_PREDEFSOUND");
		{
			...
			case PDSOUND_FORCEGRIP:
				...
				break;
			case PDSOUND_FORCEGRASP:
				sID = trap_S_RegisterSound("sound/weapons/force/telekinesis.mp3");
				break;
			default:
				break;
			}
	...
	case EV_BODYFADE:
		...
		cent->bodyFadeTime = cg.time + 20000;
		break;
