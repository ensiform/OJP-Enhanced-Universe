...
static cvarTable_t cvarTable[] = { // bk001129
	...
	{ &cg_footsteps, "cg_footsteps", "4", CVAR_ARCHIVE },
	{ &cg_swingAngles, "cg_swingAngles", "1", 0 },

	{ &cg_oldPainSounds, "cg_oldPainSounds", "1", 0 },
...
//[SFXSabers]
	{ &sfx_sabers,	"sfx_sabers", "1", CVAR_ARCHIVE },
//[/SFXSabers]

//[Movie Sabers]
	{ &cg_MovieSaberType, "cg_MovieSaberType", "0", CVAR_ARCHIVE },
//[/Movie Sabers]
...
//	CG_LoadingString( "sounds" );
	...
	for (i=1 ; i<19 ; i++)
	{
		trap_S_RegisterSound(va("sound/weapons/saber/saberblock%i", i));
	}

	for (i=1 ; i<4 ; i++)
	{
		trap_S_RegisterSound(va("sound/weapons/saber/saberknock%i", i));
	}
	...
	cgs.media.incinerationSound = trap_S_RegisterSound( "sound/effects/fire_lp.wav" );
	cgs.media.cryoSound = trap_S_RegisterSound( "sound/effects/fire_lp.wav" );
#ifdef JK2AWARDS
...
//	CG_LoadingString( "game media" );
	...
	cgs.effects.forceJudgement		= trap_FX_RegisterEffect( "effects/force/judgement.efx" );
...
//	memset( &cg, 0, sizeof( cg ) );
	...
	CG_InitJetpack2Ghoul2();

	CG_InitJetpack3Ghoul2();

	CG_InitJetpack4Ghoul2();

	CG_InitJetpack5Ghoul2();
...
//	CG_LoadFonts();
	...
	while (i <= LAST_USEABLE_WEAPON)
	{
		...
		if (item && item->icon && item->icon[0])
		{
				...
				{
				cgs.media.weaponIcons2[i] = trap_R_RegisterShaderNoMip("gfx/hud/w_icon_dh-17.tga");
				cgs.media.weaponIcons2_NA[i] = trap_R_RegisterShaderNoMip("gfx/hud/w_icon_dh-17_na.tga");
				cgs.media.weaponIcons3[i] = trap_R_RegisterShaderNoMip("gfx/hud/w_icon_clonepistol_1.tga");
				...
				{
				cgs.media.weaponIcons2[i] = trap_R_RegisterShaderNoMip("gfx/hud/w_icon_a280.tga");
				cgs.media.weaponIcons2_NA[i] = trap_R_RegisterShaderNoMip("gfx/hud/w_icon_a280_na.tga");
				cgs.media.weaponIcons3[i] = trap_R_RegisterShaderNoMip("gfx/hud/w_icon_dc-15s.tga");
				...
				{
				cgs.media.weaponIcons2[i] = trap_R_RegisterShaderNoMip("gfx/hud/w_icon_ee-3.tga");
				cgs.media.weaponIcons2_NA[i] = trap_R_RegisterShaderNoMip("gfx/hud/w_icon_ee-3_na.tga");
				cgs.media.weaponIcons3[i] = trap_R_RegisterShaderNoMip("gfx/hud/w_icon_wbowcaster1.tga");
				...
				{
				cgs.media.weaponIcons2[i] = trap_R_RegisterShaderNoMip("gfx/hud/w_icon_dlt-18.tga");
				cgs.media.weaponIcons2_NA[i] = trap_R_RegisterShaderNoMip("gfx/hud/w_icon_dlt-18_na.tga");
				cgs.media.weaponIcons3[i] = trap_R_RegisterShaderNoMip("gfx/hud/w_icon_dc-15a.tga");
				...
				{
				cgs.media.weaponIcons2[i] = trap_R_RegisterShaderNoMip(item->icon);
				cgs.media.weaponIcons2_NA[i] = trap_R_RegisterShaderNoMip(va("%s_na", item->icon));	
				cgs.media.weaponIcons3[i] = trap_R_RegisterShaderNoMip(item->icon);
		...
		{ //make sure it is zero'd (default shader)
			...
			cgs.media.weaponIcons2[i] = 0;
			cgs.media.weaponIcons2_NA[i] = 0;
			cgs.media.weaponIcons3[i] = 0;
	...
	while (i < bg_numItems)
	{
		if (bg_itemlist[i].giType == IT_HOLDABLE)
		{
			...
			{
				if (bg_itemlist[i].giTag == HI_FLAMETHROWER)
				{
	...
	while (i < NUM_FORCE_POWERS)
	{
		cgs.media.forcePowerIcons2[i] = trap_R_RegisterShaderNoMip(HolocronIcons2[i]);	
...
//	Com_Printf("... CGameside GHOUL2 Cleanup\n");
	...
	CG_CleanJetpack2Ghoul2();
	CG_CleanJetpack3Ghoul2();
