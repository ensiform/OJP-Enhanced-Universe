...
static void CG_ClipMoveToEntities ( const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end,
	...
	////lmo note, if skipNumber represents entity in the tzone, this wont be called.
...
static void Crosshair_CG_ClipMoveToEntities ( const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end,
							int skipNumber, int mask, trace_t *tr, qboolean g2Check ) {
	int			i, x, zd, zu;
	trace_t		trace, oldTrace;
	entityState_t	*ent;
...
//		if (ent->number > MAX_CLIENTS && cg.snap && ent->genericenemyindex && (ent->genericenemyindex-MAX_GENTITIES) == cg.snap->ps.clientNum)
		...
		//lmo I add trace.startsolid condition here because with duel non-x players
		// can possibly be standing inside other players, so this condition can happen!
		if((trace.fraction < 1.0 || trace.startsolid) && ent->number < MAX_CLIENTS )
		{ //trace hit a client
			if ( cg.snap->ps.duelInProgress &&
				cg.snap->ps.duelIndex != ent->number)
			{ // if you are dueling and this client is not your partner, skip it
...
void	CG_Trace( trace_t *result, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, 
	...
	t.entityNum = t.fraction != 1.0 ? ENTITYNUM_WORLD : ENTITYNUM_NONE;
	// check all other solid models
	if ( skipNumber >=0 && skipNumber < MAX_CLIENTS )
	{ // don't bother if you are in the tzone
		CG_ClipMoveToEntities (start, mins, maxs, end, skipNumber, mask, &t, qfalse);
	}
	*result = t;
}
void	Crosshair_CG_Trace( trace_t *result, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, 
					 int skipNumber, int mask ) {
	trace_t	t;
	...
	t.entityNum = t.fraction != 1.0 ? ENTITYNUM_WORLD : ENTITYNUM_NONE;
	// check all other solid models
	Crosshair_CG_ClipMoveToEntities (start, mins, maxs, end, skipNumber, mask, &t, qfalse);
...
================
*/
//RoAR mod NOTE: I added this in 1.19e
#if 0
...
void	Crosshair_CG_G2Trace( trace_t *result, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, 
					 int skipNumber, int mask ) {
	trace_t	t;
	...
	t.entityNum = t.fraction != 1.0 ? ENTITYNUM_WORLD : ENTITYNUM_NONE;
	// check all other solid models
	Crosshair_CG_ClipMoveToEntities (start, mins, maxs, end, skipNumber, mask, &t, qtrue);
